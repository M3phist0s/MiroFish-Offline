    ...
    <div class="scroll-container">
      ...
      <div class="step-card" :class="{ 'active': phase === 1, 'completed': phase > 1 }">
        ...
        <div class="card-content">
          ...
          <p class="description">
            Uses context to organize knowledge-graph entities and relationships, initialize simulated individuals, and give them unique behaviors and memories from the reality seed.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 2, 'completed': phase > 2 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            The LLM uses simulation requirements and the reality seed to set world time flow, recommendation behavior, active periods, speech frequency, event triggers, and related parameters.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 3, 'completed': phase > 3 }">
        ...
        <div class="card-content">
          <p class="api-note">POST /api/simulation/prepare</p>
          <p class="description">
            Generates initial activation events and trending topics from the narrative direction to guide the simulated world's initial state.
          </p>
      ...
      <div class="step-card" :class="{ 'active': phase === 4 }">
        ...
        <div class="card-content">
          ...
          <p class="description">Simulation environment preparation is complete. You can now start the simulation.</p>
          ...
          <div v-if="simulationConfig && autoGeneratedRounds" class="rounds-config-section">
            <div class="rounds-header">
              <div class="header-left">
                ...
                <span class="section-desc">MiroFish plans and infers <span class="desc-highlight">{{ simulationConfig?.time_config?.total_simulation_hours || '-' }}</span> hours of reality time. Each round represents <span class="desc-highlight">{{ simulationConfig?.time_config?.minutes_per_round || '-' }}</span> minutes elapsed.</span>
              </div>
              ...
              <div v-if="useCustomRounds" class="rounds-content custom" key="custom">
                <div class="slider-display">
                  ...
                  <div class="slider-meta-info">
                    <span>If agent scale is 100: estimated time approximately {{ Math.round(customMaxRounds * 0.6) }} minutes</span>
                  </div>
                </div>
              ...
              <div v-else class="rounds-content auto" key="auto">
                <div class="auto-info-card">
                  ...
                  <div class="auto-content">
                    ...
                    <div class="auto-desc">
                      <p class="highlight-tip" @click="useCustomRounds = true">For a first run, switch to Custom mode and reduce simulation rounds to preview results faster and lower error risk -></p>
                    </div>
                  </div>
                </div>
              </div>
...
watch(currentStage, (newStage) => {
  if (newStage === 'GenerateAgentPersona' || newStage === 'generating_profiles') {
    ...
    phase.value = 2
    // Enter configuration generation phase and start polling configuration.
    if (!configTimer) {
...
// Calculate auto-generated rounds from configuration instead of hardcoded defaults.
const autoGeneratedRounds = computed(() => {
  ...
  // Keep max rounds at least 40 to avoid slider range anomalies.
...
// Resolve the username for an agent_id.
...
const handleStartSimulation = () => {
  ...
  if (useCustomRounds.value) {
    // User selected custom rounds; pass max_rounds.
    params.maxRounds = customMaxRounds.value
    addLog(`Start simulation, custom rounds: ${customMaxRounds.value} rounds`)
  } else {
    // User chose auto-generated rounds; do not pass max_rounds.
...
const startPrepareSimulation = async () => {
  ...
  // Mark step 1 completed and start step 2.
  phase.value = 1
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Set the expected agent total from the prepare response.
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      if (data.progress_detail) {
        ...
        // Output detailed progress log without duplication.
        ...
        // Output message log without duplication.
...
const fetchProfilesRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Update only when the API returns valid values to avoid overwriting existing values.
      ...
      // Output persona generation progress only when the count changes.
...
const fetchConfigRealtime = async () => {
  ...
  try {
    ...
    if (res.success && res.data) {
      ...
      // Output configuration generation phase logs without duplication.
      ...
      if (data.config_generated && data.config) {
        ...
        if (data.config.time_config) {
          ...
          addLog(`Time configuration: ${tc.minutes_per_round} minutes per round, ${Math.floor((tc.total_simulation_hours * 60) / tc.minutes_per_round)} total rounds`)
        }
        ...
        stopConfigPolling()
        phase.value = 4
        addLog('✓ Environment setup completed. You can start the simulation.')
...
const loadPreparedData = async () => {
  ...
  // Get configuration using the real-time interface.
  try {
    const res = await getSimulationConfigRealtime(props.simulationId)
    if (res.success && res.data) {
      if (res.data.config_generated && res.data.config) {
        ...
        // Configuration is not yet generated; start polling.
        addLog('Configuration is generating; starting polling...')
        startConfigPolling()
      }
    }
  } catch (err) {
